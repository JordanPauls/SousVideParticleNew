...
*/
/**************************************************************************/
double Adafruit_MAX31855::readInternal(float CelsiusOffset) {							// Internal read now has celsius offset
  uint32_t v;
  ...
  internal +=CelsiusOffset;																		//Jordan added internal offset
  return internal;
}
...
*/
/**************************************************************************/
double Adafruit_MAX31855::readCelsius(float CelsiusOffset) {							// Jordan added celsius offset
  ...
  */
  ...
  // LSB = 0.25 degrees C
  centigrade *= 0.255;																	// Calculated slope for rawC to celsius conversion
  centigrade += -1.425;																	// Calculated intercept for rawC to Celsius conversion
  return centigrade;
}
...
*/
/**************************************************************************/
double Adafruit_MAX31855::readFahrenheit(float FahrenheitOffset) {					// Jordan added fahrenheit offset
  float f = readCelsius(0);
  f *= 9.0;
  f /= 5.0;
  f += 32;
  //f += FahrenheitOffset;																			// Jordan added fahrenheit offset
  return f;
}
